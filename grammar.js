...
module.exports = grammar({
    ...
    externals: ($) => [$.rust_content, $.script_content, $.style_content, $.template_text],
    ...
    rules: {
        ...
        rust_section: ($) => seq('---', optional(/\r?\n/), optional($.rust_content), '---', optional(/\r?\n/)),

        // Script section: <script>...</script>
        script_section: ($) =>
            seq('<script>', optional(/\r?\n/), optional($.script_content), '</script>', optional(/\r?\n/)),

        // Template section: <template>...</template>
        template_section: ($) => seq('<template>', optional($.template_body), '</template>', optional(/\r?\n/)),

        // Style section: <style>...</style>
        style_section: ($) =>
            seq('<style>', optional(/\r?\n/), optional($.style_content), '</style>', optional(/\r?\n/)),
        ...
        html_element: ($) =>
            choice(
                // Self-closing tag
                seq('<', field('tag_name', $.tag_name), repeat($.html_attribute), '/>'),
        ...
        client_component: ($) =>
            choice(
                ...
                seq('<', field('component_name', $.component_name), repeat($.html_attribute), '/>'),
        ...
        html_attribute: ($) =>
            seq(
                ...
                optional(seq('=', field('value', choice($.quoted_attribute_value, $.template_interpolation))))
            ),
        ...
        attribute_value_content: ($) => repeat1(choice(/[^"{}]+/, $.template_interpolation)),

        attribute_value_content_single: ($) => repeat1(choice(/[^'{}]+/, $.template_interpolation)),

        // Template interpolation: {{ expression }}
        template_interpolation: ($) => seq('{{', field('expression', $._expression), '}}'),
        ...
        else_clause: ($) => seq('{{:else}}', field('body', optional($.template_body))),
        ...
        raw_html_directive: ($) => seq('{{@html', field('content', $._expression), '}}'),
        ...
        unary_expression: ($) => prec(3, seq(field('operator', choice('!', '-')), field('operand', $._expression))),

        // Property access: obj.prop.subprop
        property_access: ($) => prec.left(2, seq(field('object', $._callable), '.', field('property', $.identifier))),

        // Function call: func(args)
        function_call: ($) =>
            prec.left(3, seq(field('function', $._callable), '(', field('arguments', optional($.argument_list)), ')')),

        _callable: ($) => choice($.identifier, $.property_access, $.function_call),
